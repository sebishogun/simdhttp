...
The module `github.com/sebishogun/simdhttp` (go 1.26.2, depends only on
`github.com/sebishogun/simd` v1.20.0, no cgo) ships exactly one thing: a
borrowed-buffer HTTP/1 request-head parser.
...
**Strictness vs net/http** — three deliberate differences, all
request-smuggling surface: space inside a field name is malformed;
bare-LF line endings are malformed; obs-fold continuation lines are
...
All confirmed by live differential against net/http on Go 1.26.2,
2026-08-13. Full records: `docs/wrong.md`; LLD `docs/lld/http1-head-parser.md`.
...
| G3 | no `Host` presence, emptiness, or format validation | HTTP/1.1 heads without Host, `Host:`, malformed hosts accepted; net/http's server rejects all |
| G4 | control bytes (NUL, DEL, …) in the request-target accepted | net/http rejects via net/url |
| G5 | no framing validation: `Content-Length` + `Transfer-Encoding`, or two `Content-Length`s, accepted | differs from net/http; smuggling surface when a server is added |
...
- the **hardened head parser** — the shipped `Parse` plus the G1–G6 fixes
  ...
  - *compatible-default*: accepts what net/http accepts (with the three
    documented strictness points), tolerant of real-world clients;
...
| field | validation |
|---|---|
| `Host` | presence (HTTP/1.1), single occurrence, `ValidHostHeader`-equivalent format |
| request-target | control bytes rejected; URI *semantics* remain the caller's (net/url), matching the fuzz contract |
| URI escapes | `%` escapes parsed by the caller; malformed escapes surfaced, not silently rewritten |
| `Content-Length` | digits only, single value, parsed exactly; multiple or differing values rejected |
| `Transfer-Encoding` | only `chunked` (final) accepted; anything else rejected; see §3.4 |
...
`Transfer-Encoding` together is always a rejection; a duplicate
`Content-Length` is a rejection even when the values agree; a
`Transfer-Encoding` that is not final `chunked` is a rejection; and the
compatible profile matches the Go server's verdicts while the strict
profile is a superset of them.
...
runs of 2026-08-13. Nothing in the target sections is measured yet — they
are design, and the plans mark each item with its verification gate.
